...
def execute_sql_statement():
    """
    English question: What are all my contacts and their email addresses, including the ones missing
    an email address and the ones with an email address but missing a contact name?
    SELECT  p.first_name, p.last_name, e.e_mail
    FROM person p FULL OUTER JOIN e_mail_address e
    ON p.key = e.person_key
    """

    sql_statement = """
    SELECT p.first_name, p.last_name, e.e_mail
    FROM person p LEFT OUTER JOIN e_mail_address e
    ON p.key = e.person_key
    UNION
    SELECT p.first_name, p.last_name, e.e_mail
    FROM e_mail_address e LEFT OUTER JOIN person p
    ON p.key = e.person_key;
    """
